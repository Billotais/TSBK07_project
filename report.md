...
To generate the end of the maze, a recusrive flood algorithm is used. It might not be optimal to have as many recursive calls, but and alternative algorithm as seen in class that iterativly flood a row, then recursivly spread to adgacent rows isn't very usefull because of the structure of the maze. Moreover, the maze isn't that big so it isn't such a problem. By keeping track of the recusrion depth, we can know what is the furthest cell from the start, and put the Holy Grail there.

...
void main(void)
{
	...
	vec3 light_coord = cameraPosition;
	...
	// Diffuse component
	float costheta = dot(normalize(transfer_normal_transformed),vertex_to_light);
	...
	vec3 vertex_to_camera = normalize(vec3(projectionMatrix*cameraMatrix*vec4(cameraPosition, 1.0)) - vec3(transfer_vertex_transformed));
